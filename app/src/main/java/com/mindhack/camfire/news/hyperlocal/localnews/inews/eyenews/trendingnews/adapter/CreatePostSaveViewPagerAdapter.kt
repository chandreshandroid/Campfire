...
class CreatePostSaveViewPagerAdapter(
    val context: Context,
    val numberOfImage: List<Albummedia>
) : PagerAdapter() {
    ...
    override fun instantiateItem(view: ViewGroup, position: Int): Any {
        ...
        if (numberOfImage?.get(position)!!.albummediaFile != null)
            imageView.setImageURI(Uri.parse(RestClient.image_base_url_post+ numberOfImage?.get(position)!!.albummediaFile),context)
        else if (!numberOfImage?.get(position)!!.albummediaFile.isNullOrEmpty())
            imageView.setImageURI(Uri.parse(RestClient.image_base_url_post+ numberOfImage?.get(position)!!.albummediaFile),context)
