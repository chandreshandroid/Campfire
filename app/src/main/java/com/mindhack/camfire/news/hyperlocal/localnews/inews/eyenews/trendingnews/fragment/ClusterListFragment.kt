...
import kotlinx.android.synthetic.main.fragment_cluster_list.*
import kotlinx.android.synthetic.main.fragment_cluster_list.feedRecyclerview
import kotlinx.android.synthetic.main.fragment_cluster_list.tollOther
...
class ClusterListFragment : Fragment() {
    ...
    private var cluster: Cluster<Person>?=null
    ...
    override fun onCreateView(
        ...
        if(v==null)
        {
            v=inflater.inflate(R.layout.fragment_cluster_list, container, false)
        }
        return v
    }
...
//        (activity as MainActivity).updateNavigationBarState(R.id.menu_home)
        ...
        if(arguments!=null)
        {
            feedList= arguments?.getSerializable("feedDataCluster") as ArrayList<TrendingFeedData?>?
        }
...
//        popup.menu.getItem(3).title = "Hide For Everyone"
    ...
    private fun getRepost(pos: Int, trendingFeedData: TrendingFeedData) {
        ...
        val repostPostModel =
            ViewModelProviders.of(this@ClusterListFragment).get(RepostPostModel::class.java)
        repostPostModel.apiFunction(mActivity!!, false, jsonArray.toString())
            .observe(this@ClusterListFragment,
...
//        lp.height = 900
            ...
            ViewCompat.animate(button).translationY((button.height).toFloat())
                ...
                .setListener(object : ViewPropertyAnimatorListener {
                    override fun onAnimationStart(view: View) {
                        this@ClusterListFragment.mIsAnimatingOut = true
                    }

                    override fun onAnimationCancel(view: View) {
                        this@ClusterListFragment.mIsAnimatingOut = false
                    }
    ...
    fun setPostViewsCount(
        ...
        try {
            if (!feedList?.get(feedPos)?.isYouViewPost?.contains("Yes")!!) {
                ...
                jsonArray.put(jsonObject)
                val postView =
                    ViewModelProviders.of(this@ClusterListFragment).get(PostViewModel::class.java)
                postView.apiCall(mActivity!!, jsonArray.toString(), "ViewPost").observe(this@ClusterListFragment,
                    androidx.lifecycle.Observer { postViewPojos ->
    ...
    private fun getUserFollow(
        ...
        var getFollowModel = ViewModelProviders.of(this@ClusterListFragment).get(FollowModel::class.java)
        getFollowModel.apiCall(mActivity!!, jsonArray.toString(), s).observe(this@ClusterListFragment,
            androidx.lifecycle.Observer { trendingFeedDatum ->
    ...
    fun setPostFavourite(
        ...
        val postLike: PostFavouriteModel =
            ViewModelProviders.of((this@ClusterListFragment)).get(
                PostFavouriteModel::class.java
            )
        postLike.apiCall(mActivity!!, jsonArray.toString(), action).observe(
            this@ClusterListFragment,
            androidx.lifecycle.Observer { postlikePojos ->
                if (mActivity != null) {
...
//        lp.height = 900
    ...
    fun getReasonList(postID: String, pos: Int) {
        ...
        val apiCall = ViewModelProviders.of(this@ClusterListFragment).get(ReasonModel::class.java)
        apiCall.apiFunction(mActivity!!, true, jsonArray.toString()).observe(this@ClusterListFragment,
            androidx.lifecycle.Observer { response ->
                if (!response.isNullOrEmpty()) {
    ...
    private fun getPostHide(
        ...
        jsonArray.put(jsonObject)
        var reportPostModel =
            ViewModelProviders.of(this@ClusterListFragment).get(ReportPostModel::class.java)

        reportPostModel.apiFunction(mActivity!!, jsonArray.toString(), "PostHide")
            .observe(this@ClusterListFragment, androidx.lifecycle.Observer { it ->
